...
void Predictor::perform() {
    auto conceptActivationFunction = ActivationFunctionsFactory().create(_predictionParameters.activationFunction, ElementType::Node, _predictionParameters.fuzzinessDegree);
    auto weightActivationFunction = ActivationFunctionsFactory().create(_predictionParameters.activationFunction, ElementType::Edge, _predictionParameters.fuzzinessDegree);
    auto algorithm = AlgorithmsFactory().create(_predictionParameters, conceptActivationFunction, weightActivationFunction);
    auto metricsManager = MetricsManager(MetricsFactory().create(_predictionParameters.metric), _predictionParameters);
    auto stopCondition = StopConditionsFactory().create(_predictionParameters);
