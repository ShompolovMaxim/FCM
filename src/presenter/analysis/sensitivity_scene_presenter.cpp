...
SensitivityScenePresenter::SensitivityScenePresenter(QCustomPlot* plot, std::shared_ptr<CreationPresenter> creationPresenter, QObject* parent)
    : ScenePresenter(parent), graphScene(nullptr), plot(plot), creationPresenter(creationPresenter) {}

SensitivityScenePresenter::~SensitivityScenePresenter() {
    stopExecution();
    closeRuntimeWindows();
}

void SensitivityScenePresenter::setRuntimeContext(std::shared_ptr<FCM> originalFcm_, std::shared_ptr<FCM> runtimeFcm_, GraphScene* runtimeScene) {
    closeRuntimeWindows();
...
void SensitivityScenePresenter::updateConcept(QUuid id, ElementWindowMode mode) {
...
void SensitivityScenePresenter::updateWeight(QUuid id, ElementWindowMode mode) {
...
void SensitivityScenePresenter::emitAutosave() {}

void SensitivityScenePresenter::analize(PredictionParameters predictionParameters, SensitivityAnalysisParameters parameters) {
    stopExecution();
    ...
    connect(timer, &QTimer::timeout, this, [this, parameters]() {
        ...
        if (parameters.changeConcepts) {
            for (auto& [id, concept] : runtimeFcm->concepts) {
                ...
                graphScene->setConceptColor(id, maxSensitivity ? DefaultColorValueAdapter().getColor(std::min(analizer->getConceptSensitivity(id), 1.0) / maxSensitivity, 0, 1) : red, false);
        ...
        if (parameters.changeWeights) {
            for (auto& [id, weight] : runtimeFcm->weights) {
                ...
                graphScene->setWeightColor(id, maxSensitivity ? DefaultColorValueAdapter().getColor(std::min(analizer->getWeightSensitivity(id) / maxSensitivity, 1.0), 0, 1) : red);
...
void SensitivityScenePresenter::reset() {
    stopExecution();
    active = false;
...
void SensitivityScenePresenter::stopExecution() {
    if (timer) {
        timer->stop();
...
void SensitivityScenePresenter::closeRuntimeWindows() {
...
void SensitivityScenePresenter::openRuntimeConceptWindow(QUuid id, ElementWindowMode mode) {
    if (!runtimeFcm) {
        return;
    }
...
void SensitivityScenePresenter::openRuntimeWeightWindow(QUuid id, ElementWindowMode mode) {
    if (!runtimeFcm) {
        return;
    }
...
void SensitivityScenePresenter::updateRuntimeConceptWindow(QUuid id) {
...
void SensitivityScenePresenter::updateRuntimeWeightWindow(QUuid id) {
